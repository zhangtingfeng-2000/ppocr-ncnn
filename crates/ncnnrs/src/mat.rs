...
impl Mat {
    ...
    pub fn data_as_slice<T>(&self, len: usize) -> &[T] {
        let byte_len = len.saturating_mul(size_of::<T>());
        assert!(byte_len <= self.cstep() * self.c().max(1) as usize * self.elemsize() as usize);
